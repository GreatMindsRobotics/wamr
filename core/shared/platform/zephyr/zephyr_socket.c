...
os_socket_set_ip_multicast_loop(bh_socket_t socket, bool ipv6, bool is_enabled)
{
    if (ipv6) {
        ...
        return os_socket_setbooloption(socket, IPPROTO_IP, IP_MULTICAST_LOOP,
                                       is_enabled);
#else
        /* Zephyr 4.1+ no longer defines IP_MULTICAST_LOOP in its socket
         * headers. The firmware doesn't use multicast either way;
         * LIBC_WASI just pulls this code path into the compile. */
        errno = EAFNOSUPPORT;
        return BHT_ERROR;
#endif
    }
